...
import { getDictListApi } from "@/api/wiki/zy";
import type {
  DiseaseEntityType,
  AlphabetEntityType,
  CategoryEntityType
...
// 按专科的目录数据
const categories = ref<Array<CategoryEntityType>>([
  { id: "all", name: "全部" } as any
]);

// 按首字母的目录数据
const alphabets = ref<Array<AlphabetEntityType>>([
  { id: "all", name: "全部" } as any
]);
...
const filteredDiseases = computed(() => {
  ...
  // Filter by search query (支持中文名、拼音全拼、拼音首字母)
  if (searchQuery.value.trim()) {
    const keyword = searchQuery.value.toLowerCase();
    filtered = filtered.filter(
      disease =>
        disease.name.toLowerCase().includes(keyword) || // 中文匹配
        disease.pinyin.toLowerCase().includes(keyword) || // 拼音全拼
        disease.pinyinInitial.toLowerCase().includes(keyword) // 拼音首字母
    );
  }

  // Filter by category/alphabet
...
const loadDictData = async () => {
  try {
    ...
    if (res.code === 0 && res.data) {
      // 赋值科室列表（在"全部"后面添加后端返回的科室）
      categories.value = [
        { id: "all", name: "全部" } as any,
        ...res.data.categories
      ];

      // 赋值首字母列表（在"全部"后面添加后端返回的首字母）
      alphabets.value = [
        { id: "all", name: "全部" } as any,
        ...res.data.alphabets
      ];
