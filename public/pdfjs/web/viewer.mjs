...
const {
  ...
  version,
  XfaLayer
} = globalThis.pdfjsLib; // CONCATENATED MODULE: ./web/event_utils.js
...
const GenericL10n = null; // CONCATENATED MODULE: ./web/genericl10n.js
...
class PDFDocumentProperties {
  ...
  async #parsePageSize(pageSizeInches, pagesRotation) {
    ...
    const [{ width, height }, unit, name, orientation] = await Promise.all([
      ...
      this.l10n.get(
        `pdfjs-document-properties-page-size-unit-${this._isNonMetricLocale ? "inches" : "millimeters"}`
      ),
      rawName &&
        this.l10n.get(`pdfjs-document-properties-page-size-name-${rawName}`),
      this.l10n.get(
        `pdfjs-document-properties-page-size-orientation-${isPortrait ? "portrait" : "landscape"}`
      )
    ]);
    return this.l10n.get(
      `pdfjs-document-properties-page-size-dimension-${name ? "name-" : ""}string`,
      {
...
class PDFViewer {
  ...
  _setCurrentPageNumber(val, resetCurrentPageView = false) {
    ...
    // 页码变化时，触发事件
    if (window._afterPageChange) {
      window._afterPageChange(val);
    }
    return true;
  }
  ...
  async _parseHashParams() {
    ...
    const loadPDFBug = async () => {
      ...
      const { PDFBug } = await import(
        /*webpackIgnore: true*/ AppOptions.get("debuggerSrc")
      );
      this._PDFBug = PDFBug;
    };
...
var __webpack_exports__PDFViewerApplication =
  __webpack_exports__.PDFViewerApplication;
var __webpack_exports__PDFViewerApplicationConstants =
  __webpack_exports__.PDFViewerApplicationConstants;
var __webpack_exports__PDFViewerApplicationOptions =
